...
while (pos < buffer.length - 4) {
  ...
  if (buffer[pos] === 0xFF && buffer[pos+1] === 0xD8 && buffer[pos+2] === 0xFF) {
    ...
    if (foundEnd) {
      ...
      // Filter out small thumbnails (< 15KB)
      if (imgBuffer.length > 15000) {
        count++;
        const filename = `netter_unlabeled_plate_${String(count).padStart(3, '0')}.jpg`;
        const filePath = path.join(outputDir, filename);
        fs.writeFileSync(filePath, imgBuffer);
        console.log(`Extraída prancha Netter sem etiquetas #${count}: ${filename} (${Math.round(imgBuffer.length / 1024)} KB)`);
      }
      pos = endPos;
      continue;
    }
