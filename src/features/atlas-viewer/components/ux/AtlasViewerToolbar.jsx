...
export default function AtlasViewerToolbar({ 
  ...
  return (
    <div className="absolute bottom-[env(safe-area-inset-bottom,16px)] sm:bottom-6 left-1/2 -translate-x-1/2 z-30 flex items-center gap-1 sm:gap-2 p-1.5 sm:p-2 bg-[#0B0E14]/70 backdrop-blur-xl border border-white/10 rounded-xl shadow-[0_20px_40px_rgba(0,0,0,0.5)] max-w-[95vw] overflow-x-auto overflow-y-hidden [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      ...
      {/* Markers Toggle */}
      <AtlasTooltip content={isSketchfabMode ? "Recurso nativo (Indisponível no Sketchfab)" : isMarkerPanelOpen ? "Fechar marcadores" : "Abrir marcadores"} position="top">
        <button 
          onClick={isSketchfabMode ? null : toggleMarkerPanel}
          className={`relative h-10 px-4 rounded-lg border transition-all flex items-center gap-2 ${
            isSketchfabMode 
              ? 'bg-[#151A23]/50 border-white/5 text-slate-600 cursor-not-allowed'
              : isMarkerPanelOpen 
      ...
      {/* Quality */}
      <AtlasQualityToggle renderMode={renderMode} setRenderMode={setRenderMode} disabled={isSketchfabMode} />
      
      <div className="w-px h-6 bg-white/10 mx-1"></div>

      {/* Study Mode */}
      <AtlasTooltip content={isSketchfabMode ? "Recurso nativo (Indisponível no Sketchfab)" : "Modo estudo"} position="top">
        <button 
          onClick={isSketchfabMode ? null : toggleStudyMode}
