                          ...
                          limit(lim: number) {
                            if (table === "ai_messages") {
                              const filtered = db.messages
                                .filter((m) => (m as any)[col1] === val1 && (m as any)[col2] === val2);
                              // Se ascending for false, ordena do mais novo para o mais antigo (created_at DESC)
                              const sorted = opts?.ascending === false
                                ? [...filtered].reverse()
                                : [...filtered];
                              const found = sorted.slice(0, lim);
